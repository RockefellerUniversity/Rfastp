...
void Stats::print() {
    Rcpp::Rcerr << "Total reads: " << mRead << endl;
    Rcpp::Rcerr << "Total bases: " << mBase << endl;
    Rcpp::Rcerr << "Mapped reads: " << getMappedReads() << " (" << to_string( getMappedReads()*100.0/mRead ) << "%)" << endl;
    Rcpp::Rcerr << "Mapped bases: " << getMappedBases() << " (" << to_string( getMappedBases()*100.0/mBase ) << "%)" << endl;
    Rcpp::Rcerr << "Bases mismatched with reference: " << mBaseMismatches << " (" << to_string( mBaseMismatches*100.0/getMappedBases() ) << "%)" <<  endl;
    Rcpp::Rcerr << "Reads with mismatched bases: " << mReadWithMismatches << " (" << to_string( mReadWithMismatches*100.0/getMappedReads() ) << "%)" <<  endl;
    Rcpp::Rcerr << "Total mapping clusters: " << mCluster << endl;
    Rcpp::Rcerr << "Mapping clusters with multiple fragments: " << mMultiMoleculeCluster << endl;
    Rcpp::Rcerr << "Total fragments: " << mMolecule << endl;
    Rcpp::Rcerr << "Fragments with single-end reads: " << mMoleculeSE << endl;
    Rcpp::Rcerr << "Fragments with paired-end reads: " << mMoleculePE << endl;
    Rcpp::Rcerr << "Duplication level histogram: " << endl;
    for(int i=1; i<MAX_SUPPORTING_READS && i<=10; i++) {
        if(mSupportingHistgram[i] == 0)
            break;
        Rcpp::Rcerr << "    Fragments with " << i << " duplicates: " << mSupportingHistgram[i] << endl;
    }
}
